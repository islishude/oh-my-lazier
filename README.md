...
- Before durable loops and database initialization, worker startup performs one bounded, concurrent pricing-source identity check: it probes configured market IDs, requires Chainlink descriptions to match, requires Uniswap pools to expose the configured token pair, and confirms that each configured Uniswap TWAP window has enough observation history. Stable request/identity mismatches, successful market-data responses that omit the requested ID, contract-call reverts during identity checks, an Uniswap `OLD` observation-history revert, empty or malformed ABI responses from otherwise successful identity calls, malformed or insecure authenticated market-data BaseURLs, and missing or malformed secret references fail fast. Market-data HTTP 403/404 responses, non-`OLD` Uniswap observation failures, timeouts, transport/RPC errors, rate limits, and upstream server failures are deferred to the supervised runtime loops, so transient pricing outages do not terminate unrelated worker loops. Authenticated market-data endpoints must use HTTPS, and their clients do not follow redirects to another origin.
