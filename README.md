...
- Indexer polling errors are logged, exposed through process-local metrics, and retried without advancing the failed stream cursor. Other durable loop errors are logged and supervised with restart backoff instead of canceling the worker process.
