...
- Transaction type is derived at send time from the latest block header: headers with `baseFee` use EIP-1559 dynamic-fee transactions, while headers without `baseFee` use legacy gas-price transactions. The tx manager reads current RPC gas/tip suggestions and estimates outer transaction gas before signing each outbox row; configured `max_fee_per_gas_wei` is the send cap for both legacy gas price and dynamic `GasFeeCap`, and `max_priority_fee_per_gas_wei` is required only when the chain header supports dynamic fees.
- Address fields in worker config are parsed as EVM 20-byte hex addresses at load time. `chains[].tx_roles` defines local transaction signers and fee caps for each destination chain. `pricing.base_fee_wei` is the worker contract quote base fee, not the EIP-1559 block base fee. `pathways[].source_workers` defines the OpenExecutor/OpenDVN contracts selected by the source pathway.
