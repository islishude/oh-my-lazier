...
- Worker startup constructs pricing readers without probing live market-source health. It still rejects malformed market-data BaseURLs and missing API-key environment values as local configuration errors. Source reads remain fail-closed inside supervised runtime loops, so transient pricing outages do not terminate unrelated worker loops.
