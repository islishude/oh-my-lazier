...
- Per-chain `start_block_number` is optional and defaults to `0`. It only seeds the first indexer backfill when no durable cursor exists; after a cursor is written, the database cursor is authoritative. Per-chain `indexer_query_block_range` defaults to `500` and caps each source-chain `FilterLogs` request. Live log subscriptions start from the block after the startup confirmed frontier and only wake the cursor-backed backfill loop.
