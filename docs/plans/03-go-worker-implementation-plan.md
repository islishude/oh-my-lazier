...
- [~] DVN shadow/active flow verifies source evidence: mode、loop boundary、DVN `PacketSent` / `DVNFeePaid` / `DVNJobAssigned` decode/correlation、destination ReceiveUln302 `PayloadVerified` decode/indexing、`dvn_jobs` persistence、confirmation wait transition、source head/receipt/log verification、healthy-provider head/receipt comparison、RPC conflict persistence、block-hash conflict chain pause、receipt/log conflict pathway pause、shadow `WOULD_VERIFY` report persistence、active `ReceiveUln302.verify` outbox enqueue 与 successful receipt or indexed `PayloadVerified` -> `VERIFIED` transition exist; testnet shadow/active validation remains pending.
