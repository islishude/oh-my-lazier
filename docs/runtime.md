...
- An underpriced raw is automatically repriced (10% bump under the configured caps) after a one-minute cooldown; a node's deterministic rejection or an exhausted signing budget parks the signer lane as `held` for operator review instead of releasing the nonce, while an accepted broadcast that reaches the automatic replacement cap stays under receipt polling and emits an actionable recovery alert.

- Never-accepted business transactions rejected as `retryable_env` replay the same persisted raw after 20, 40, 80, and 160 seconds (five broadcasts total, roughly five minutes plus RPC time). Cancel attempts and other send classes keep their existing pacing; prior acceptance is never revoked. Exhaustion parks `held(broadcast_exhausted)`. If the last send class is `retryable_env`, there is no cancel intent, and signing/replacement budgets remain, the lowest-nonce row enters automatic recovery after a persisted 60-second cooldown. Every historical attempt receipt is checked first; unexplained nonce consumption routes to reconciliation. Visibility resumes `broadcast`; replacement requires quorum absence spanning at least 60 seconds and visibility/nonce evidence at most two minutes old. Replacements preserve the intent and nonce, bump fees at least 10% within configured caps, and stop at five automatic replacements. Unknown classes and other holds gain no new automatic authorization. Explicit operator replacement requests take priority over environmental probes, including probes already in flight. Restart preserves timers and budgets; existing eligible holds recover through normal probes without a data backfill. See [transaction recovery](runbooks/monitoring.md#environmental-broadcast-recovery) for readiness and operator actions.
