...
| [~]  | 5. 实现 signer 与 tx manager                                   | signer inventory config、本地 keystore signer、AWS KMS signer、tx manager loop、App tx target wiring、tx_outbox enqueue、advisory-lock nonce assignment、EIP-1559 签名广播、replacement tx 与 manual retry CLI 已有；`make test-kms-rustack` 提供显式 Rustack KMS 集成测试入口，当前外部 mock 不支持 `ECC_SECG_P256K1` 时会 skip                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| [~]  | 12. 完成迁移演练并编写主网 runbook                             | DB-backed `/healthz`/`/readyz`/`/metrics` endpoint、monitoring checklist、validated config diff CLI/runbook、DB-backed readiness check CLI、key management review、price bot review、rate-limit review、mainnet readiness runbook、迁移 evidence JSON checker/template、parent-agent security review、Codex Security preflight ready 结果与 `make security-check` 已有；npm audit critical 已清零，剩余 6 个 high 和 4 个 moderate transitive LayerZero/Hardhat toolbox advisories、exhaustive security review 与最终主网 approval 待后续阶段                                                                                                                                                                                                                                                                                                                           |

## 当前开放项

- 已执行 `npm install --save-exact ...@latest`，当前精确版本记录在 `plans/02-contracts-implementation-plan.md`。
- 测试网执行前运行 `npm run check:lz-addresses`，重新确认 `docs/deployments/layerzero-testnet-addresses.md` 中的外部 LayerZero 地址仍是最新。
