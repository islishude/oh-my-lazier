...
- `go/internal/db` stores per-chain signer nonce cursors in Postgres. The tx manager initializes a missing cursor from RPC pending nonce on first use, then increments only the local cursor. Automatic failed-transaction retry follows the same cursor: replacement keeps the assigned nonce, while fresh receipt retry consumes the next local nonce. Broadcast rows without a receipt for `tx_manager.stale_broadcast_replacement_after_seconds` seconds are automatically re-signed with the same nonce and replacement fee bump until the attempt cap is reached. After bootstrap, do not use the same signer account from another broadcaster.
