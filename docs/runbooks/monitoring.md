...
- `LazWorkerReadinessFailed`: `/readyz` returns non-200 for more than two scrape intervals; page. Readiness also fails while any active-chain signer lane is parked `held(manual)`, `held(nonce_consumed_externally)`, `held(broadcast_exhausted)` without automatic recovery eligibility, or reprice-held past the automatic replacement cap (reported as the synthetic `reprice_exhausted` reason) — those holds block every higher nonce for the signer until the operator resolves them with `txretry`; the self-healing `broadcast_retrying` and `reprice_required` (below the cap) and `nonce_reconcile_required` holds and fresh pending cancels do not fail readiness — except a `broadcast_retrying` or `reprice_required` hold or a pending cancel older than fifteen minutes, which means recovery has stalled (check RPC availability, signer balance and configured fee caps) and also fails readiness. The cancel age counts from the immutable `cancel_requested_at`; deferrals only move the separate pacing column, so a fee-cap-stuck cancel cannot hide behind a perpetually young age.
...
- `LazIndexerPollStalled`: compare each `chain_eid`/`stream` `laz_indexer_last_poll_timestamp_seconds`, or `laz_indexer_start_timestamp_seconds` before its first completion, with twice `laz_indexer_poll_interval_seconds`; page after the condition persists for five minutes. This catches one independent stream whose initial or later poll stops completing without treating regularly completed failed polls as a stall.
...
- Indexer configured poll interval, loop start timestamp, poll success, current failure start, last completed poll, last success, last poll duration, observed head, safe block upper bound, and last error timestamp by chain and stream: `laz_indexer_poll_interval_seconds`, `laz_indexer_start_timestamp_seconds`, `laz_indexer_poll_success`, `laz_indexer_failure_since_timestamp_seconds`, `laz_indexer_last_poll_timestamp_seconds`, `laz_indexer_last_success_timestamp_seconds`, `laz_indexer_last_poll_duration_seconds`, `laz_indexer_observed_head_block`, `laz_indexer_safe_to_block`, and `laz_indexer_last_error_timestamp_seconds`. The poll interval is the caught-up, pending, and retry wait; lagging streams run consecutive backfill passes immediately.
- RPC provider quorum classification by chain and redacted provider id: `laz_rpc_provider_status` (`healthy`, `lagging`, `conflict`, `unavailable`) plus the sticky per-provider `laz_rpc_provider_safe_conflict`, `laz_rpc_provider_log_conflict`, and `laz_rpc_provider_state_conflict` flags. Reported after every indexer poll and every signer balance poll; the balance poll runs its own head quorum probe before reporting, so deployments without indexers (for example pricing-only) still feed the head conflict and quorum-unavailable alerts with live classifications rather than the startup snapshot.
...
- Indexer logs emit at most one throttled `Info` `indexer progress` entry per chain and stream per interval, including its block range, lag, pending state, processed item counts, and duration. Per-window `indexer stream advanced` entries and per-poll `indexer poll completed` summaries are `Debug`. Per-event entries identify source assignments and destination reconciliation events by `guid`, `src_eid`, `dst_eid`, and `tx_hash`.
- Tx manager logs identify nonce bootstrap/claim, signing, broadcast, receipt confirmation/failure, mined gas usage, actual destination-chain gas cost, and retry enqueue by `tx_outbox_id` or `id`, `chain_eid`, `chain_name`, `signer`, and `purpose`; every log that contains `chain_eid` also contains the configured `chain_name`. They must not include calldata, signatures, keystore contents, or raw secret-bearing config.
- RPC failures and quorum conflicts identify configured endpoints as `provider[n]`, where `n` is the zero-based configuration order. Go transport errors redact RPC URLs, credentials, paths, and query values. Typed JSON-RPC error codes and messages are deliberately shown in full, even if an upstream echoes sensitive text; `error.data` is not printed. See the [RPC diagnostic policy](../runtime.md#rpc-quorum) for the output boundary used by logs and persisted job errors.
- RPC head, safe-block, log, account-nonce, and receipt-canonicality reads require a fixed strict majority of the configured providers (`q = floor(N/2) + 1` over all configured endpoints, not over the currently reachable subset). The indexer advances only through the highest `safe` height and block hash supported by that majority; unsupported or disagreeing safe responses never fall back to `latest`, `finalized`, or a configured depth. The nonce read feeds nonce reconciliation, which parks signer lanes and advances the durable nonce cursor, so a single fabricated `eth_getTransactionCount` answer can never drive it: without a majority value the reconciliation round is skipped and durable state stays untouched. A minority provider that forks or serves divergent safe blocks or logs is classified `conflict` (head) or flagged by the corresponding sticky conflict metric without stopping the worker; when tip candidates diverge and the head vote only succeeds at a lower height, every provider whose tip is above that verified height is classified `unavailable` for the round, so single-source latest reads and sends never route to an unverified fork descendant; losing the majority itself stops reads fail-closed and surfaces as failed polls and stalled cursors, never as ingesting unverified data.
...
## Environmental broadcast recovery

`retryable_env` includes `insufficient funds`, a full txpool, and account limits.
These errors can be genuine resource shortages or a provider inconsistency; they
are not evidence that the signer key was used externally. `ambiguous` means a send
may have been accepted, not that an environmental rejection was proven.

Never-accepted business attempts retain a five-broadcast budget, with waits of
20, 40, 80, and 160 seconds. A fifth rejection enters `held(broadcast_exhausted)`.
The latest `tx_attempts.send_error_class` is already durable; it records only the
last result, not a history of every send. With a final `retryable_env`, no cancel
intent, and remaining signing/replacement budget, the head nonce recovers after a
60-second cooldown. The worker checks every historical receipt and the account
nonce. Visible transactions return to `broadcast`; unseen ones need quorum absence
for at least 60 seconds, with both visibility and nonce evidence no older than two
minutes, before a same-nonce replacement. RPC errors never count as absence.

Recovery preserves all existing fee caps, confirmation-depth checks and nonce
barriers. Each replacement has a fresh five-broadcast budget; at most five automatic
replacements are allowed. Fee-cap deferrals do not consume signing-success budget.
Cancel transactions retain their existing recovery policy. Restarts preserve the
durable timers and counters; upgrading also probes existing eligible holds, without
rewriting migration history or backfilling data.

Metrics report eligible holds as the synthetic reason `broadcast_retrying` while
the stored held reason stays `broadcast_exhausted`. Age counts from the active
attempt's creation, so probes and deferrals cannot hide a stalled recovery.
`LazTxBroadcastRetrying` warns after five minutes. These holds do not alone fail
readiness until their age exceeds fifteen minutes; existing business checks such
as stalled pricing still apply. Ineligible or budget-exhausted holds retain
`broadcast_exhausted`, fail readiness, and trigger `LazTxBroadcastExhausted` (page).
Both broadcast alerts join `laz_chain_enabled == 1` by chain and scrape identity
(`job`, `instance`), so disabled chains do not alert and another worker cannot
make their retained records actionable. Re-enabling a chain restores its alerts.

Use `txretry -action inspect -id <tx_outbox_id>` to review send class, attempts and
budgets. Check RPC availability, signer balance and fee caps; after correcting the
cause, explicitly authorize `replace`, `rebroadcast`, or `cancel-nonce` as appropriate.
An explicit `replace` request takes priority over automatic environmental probes,
including when fee preflight has deferred that request. A probe already in flight
cannot overwrite the request's schedule or hold state; receipt and replacement
preflight checks still apply.
Reserve the key-access audit and `resolve-external-nonce` procedure for
`nonce_consumed_externally`; an environmental rejection alone does not justify it.
