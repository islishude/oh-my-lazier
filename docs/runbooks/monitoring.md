...
- Indexer logs emit one `indexer stream advanced` entry per advanced stream with `from_block`, `to_block`, `confirmed_to_block`, `lag_blocks`, and processed item counts, followed by an `indexer poll completed` summary for the chain. Per-event entries identify source assignments and destination reconciliation events by `guid`, `src_eid`, `dst_eid`, and `tx_hash`.
