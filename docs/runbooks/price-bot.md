...
- CoinMarketCap uses a numeric asset `id`, the V3 quotes endpoint, and `coinmarketcap_api_key_env`; never put the key itself in YAML. CoinGecko uses its coin `id`, requests `last_updated_at`, and optionally reads a Pro key through `coingecko_api_key_env`. Secret environment-variable references must match `^[A-Z_][A-Z0-9_]*$`; local validation rejects malformed references without echoing their values. When the CoinGecko key reference is set and `coingecko_base_url` is omitted, the worker selects the CoinGecko Pro endpoint automatically; an explicit BaseURL still overrides the default. A BaseURL used with either API key must use HTTPS. Both HTTP readers cap each response body at 1 MiB, reject oversized payloads, and refuse redirects to another origin so custom authentication headers cannot be forwarded to the redirect target.
...
Before database initialization, long-running worker startup performs one bounded, concurrent source-identity preflight. It probes configured market IDs, verifies Chainlink feed descriptions, verifies Uniswap pool token pairs, and asks each Uniswap pool to serve the configured TWAP window. Stable market request failures and confirmed chain identity mismatches fail fast; a successful CoinMarketCap or CoinGecko response that omits the requested ID is the deterministic invalid-ID signal and also fails fast. An identity call that reverts, returns no data, or returns data that cannot be decoded with the expected Chainlink/Uniswap ABI is a deterministic wrong-contract error. For the Uniswap observation probe, only a confirmed `OLD` revert is treated as a deterministic too-short-history mismatch. Market-data HTTP 403/404 responses, other Uniswap observation failures, timeouts, transport/RPC errors, rate limits, and upstream server failures are logged and deferred to the supervised pricing and fee-accounting loops. The preflight does not require current price freshness or gas-price health. Deferred failures remain fail-closed for the affected cycle, enqueue no partial snapshot, and restart only the affected loop instead of terminating the worker process.
