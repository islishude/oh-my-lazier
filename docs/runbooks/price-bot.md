...
- CoinMarketCap uses a numeric asset `id`, the V3 quotes endpoint, and `coinmarketcap_api_key_env`; never put the key itself in YAML. CoinGecko uses its coin `id`, requests `last_updated_at`, and optionally reads a Pro key through `coingecko_api_key_env`. When that key reference is set and `coingecko_base_url` is omitted, the worker selects the CoinGecko Pro endpoint automatically; an explicit BaseURL still overrides the default. Both HTTP readers cap each response body at 1 MiB and reject oversized payloads.
