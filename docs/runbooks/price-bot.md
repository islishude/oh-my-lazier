...
- Supported cross-asset primary sources are `coinmarketcap` and `coingecko`. `sanity_sources` may use those sources plus `uniswap`, must include `uniswap`, and must not duplicate the primary source.
