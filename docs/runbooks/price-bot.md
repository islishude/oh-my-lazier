...
Before database initialization, long-running worker startup performs one bounded, concurrent source-identity preflight. It probes configured market IDs, verifies Chainlink feed descriptions, and verifies Uniswap pool token pairs. Stable market request failures and confirmed chain identity mismatches fail fast; an identity call that reverts, returns no data, or returns data that cannot be decoded with the expected Chainlink/Uniswap ABI is a deterministic wrong-contract error and also fails fast. Market-data HTTP 403/404 responses and successful responses that omit the requested ID are availability failures because providers can return them transiently. Those cases, timeouts, transport/RPC errors, rate limits, and upstream server failures are logged and deferred to the supervised pricing and fee-accounting loops. The preflight does not require current price freshness or gas-price health. Deferred failures remain fail-closed for the affected cycle, enqueue no partial snapshot, and restart only the affected loop instead of terminating the worker process.
