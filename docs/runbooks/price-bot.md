...
Long-running worker startup constructs the configured price readers but does not make a live market-source or pricing gas-read preflight. Reader construction still rejects malformed market-data BaseURLs and missing API-key environment values as local configuration errors. The first live reads happen inside the supervised pricing and fee-accounting loops after database initialization. A transient source failure remains fail-closed for that cycle, enqueues no partial snapshot, and restarts only the affected loop instead of terminating the worker process.
