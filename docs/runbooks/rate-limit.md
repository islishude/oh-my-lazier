...
3. For drain, set `RATE_LIMIT_CAPACITY=0` and `RATE_LIMIT_REFILL_PER_SECOND=0` or call `pauseSend`.
4. Wait for in-flight packets to deliver before changing DVN required sets.
5. After validation, restore the approved rate limit and unpause sends.
...
2. Inspect in-flight packets and tx outbox state.
