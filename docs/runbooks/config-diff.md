...
- Confirm pricing `native_asset_id`, feed source, market-data BaseURL, pathway-scoped worker fee model, stale threshold, gas spike threshold, fee cap changes, and signer `min_native_balance_wei` thresholds are expected. Market-data BaseURL values are redacted in the artifact. Pathways that share a source worker and destination EID must keep that worker role's fee model identical. Outer transaction gas is estimated by the tx manager at send time.
