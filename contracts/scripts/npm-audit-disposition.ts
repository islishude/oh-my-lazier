...
if (errors.length > 0) {
  throw new Error(`npm audit disposition check failed:\n${errors.join("\n")}`);
}
...
console.log(
  `npm audit disposition ok: critical=0, disposed high/moderate findings=${openModerateOrHigh.length}, total=${counts.total ?? 0}`,
);
